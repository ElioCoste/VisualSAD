...
class AVADataset:
    ...
    def get_intersection(self):
        ...
        """
        for mode in self.modes:
            self.file_ids[mode] = list(
                set(self.file_ids[mode]) & set(self.csv_ids[mode]))
            new_file_names = []
        ...
        if subset_exists:
            ...
            for mode in self.modes:
                with open(os.path.join(self.annotations_dir, f"{mode}_subset_file_list.txt"), "r") as f:
                    self.file_names[mode] = list(
                        map(lambda line: line.strip(), f.readlines()))
                    self.file_ids[mode] = list(
                        map(lambda line: line.split(".")[0], self.file_names[mode]))
        else:
            subset_names_list = dict.fromkeys(self.modes)
            for mode in self.modes:
                subset_names_list[mode] = random.choices(
                    self.file_names[mode], k=subset_nb[mode])
                self.file_names[mode] = subset_names_list[mode]
                self.file_ids[mode] = list(
                    map(lambda line: line.split(".")[0], subset_names_list[mode]))
    ...
    def get_csv_ids(self):
        ...
        for mode in self.modes:
            df_dict[mode] = pd.read_csv(os.path.join(
        ...
        with open(os.path.join(self.annotations_dir, f'{mode}_file_list.txt'), "r") as f:
            if mode == "trainval":
                ...
                trainval_names = list(
                    map(lambda line: line.strip(), f.readlines()))
                trainval_ids = list(
                    map(lambda line: line.split(".")[0], trainval_names))
    ...
    def download_file(self, filename, mode):
        ...
        if os.path.isfile(os.path.join(self.video_dir, mode, filename)):
            ...
            if filename in self.file_names[mode]:
                open(os.path.join(self.video_dir, mode,
...
def main(use_subset=True, subset_exists=False, train_subset=20, val_subset=10, test_subset=5):
    ...
    if subset_exists:
        ...
        if use_subset:
            dataset.get_subset(subset_nb=subset_nb,
                               subset_exists=subset_exists)
...
if __name__ == "__main__":
    ...
    argparse.add_argument("--use_subset", type=bool,
                          default=True, help="Use subset of the dataset")
    argparse.add_argument("--subset_exists", type=bool,
                          default=False, help="Use existing subset of the dataset")
    argparse.add_argument("--train_subset", type=int,
                          default=2, help="Number of training samples")
    argparse.add_argument("--val_subset", type=int, default=2,
                          help="Number of validation samples")
    argparse.add_argument("--test_subset", type=int,
                          default=2, help="Number of test samples")
    ...
    main(use_subset=use_subset, subset_exists=subset_exists,
         train_subset=train_subset, val_subset=val_subset, test_subset=test_subset)
